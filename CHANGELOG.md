...
- 新增 `app.py`：Streamlit 多轮对话式推荐主界面，含场景识别、预算/技术水平选择、Top 3 推荐卡片、横向对比表、成本估算
- 新增 `data/tools.yaml`：20+ 款主流 AI 工具数据库，覆盖编程/写作/图像/语音/搜索/数据/视频 7 大类别，含评分、价格、适用场景
- 新增 `src/recommender.py`：推荐引擎，支持场景分类、预算过滤、技术水平匹配、综合评分排序
- 新增 `src/exporter.py`：报告导出模块，支持 Markdown 和 PDF（fpdf2）格式
- 新增 `src/__init__.py`：src 包初始化
- 新增 `requirements.txt`：依赖声明（streamlit、pyyaml、fpdf2）
- 新增 `config.yaml`：项目配置文件，含版本号和应用参数
...
- 生成 `AGENTS.md`（跨平台 AI 项目指令，含项目目标、工作流、工程原则）
- 生成 `README.md`（项目介绍、工具覆盖范围、目录结构说明）
- 配置项目工程原则和工作流（Python + Streamlit 技术栈）
